...
const main = async function () {
  ...
  const input = {
    messages: [new HumanMessage("Tell me about the 49ers NFL team.")],
  };

  const eventStream = graph.streamEvents(input, config);
  // Iterate over the streamed tokens and print them
...
if (process.argv[1].endsWith("streaming-tokens.ts")) {
