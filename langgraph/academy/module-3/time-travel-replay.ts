...
const main = async function () {
  ...
  const toReplay = allStates[allStates.length - 2];
  console.log(toReplay);
  console.log(toReplay.config);

  // Replay the graph from a specific checkpoint
  for await (const chunk of await graph.stream(null, toReplay.config)) {
    console.log(chunk);
    console.log("\n====\n");
  }
};
...
if (process.argv[1].endsWith("time-travel-replay.ts")) {
