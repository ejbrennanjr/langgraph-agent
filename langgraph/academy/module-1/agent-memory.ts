...
const main = async function () {
  ...
  // First invocation:
  const stateFromAddCall = await graph.invoke(
    {
      messages: new HumanMessage("Add 3 and 4."),
    },
    config
  );
  console.log(stateFromAddCall); // Log the response from the LLM.

  // Second invocation:
  const stateFromMultiplyCall = await graph.invoke(
    {
      messages: new HumanMessage("Multiply that by 2."),
    },
    config
  );
  console.log(stateFromMultiplyCall); // Log the response from the LLM.
};

// === Script Execution Section ===
// Conditionally run `main()` only when the script is executed directly (not imported into another module).

if (process.argv[1].endsWith("agent-memory.ts")) {
