...
const chatModelNode = async (state: typeof MessagesAnnotation.State) => {
  ...
  return { messages: [await llm.invoke(trimmedMessages)] };
};
...
const main = async function () {
  ...
  // const exampleTrimMessages = await trimMessages(messages, {
  //   maxTokens: 30,
  //   strategy: "last",
  //   tokenCounter: new ChatOpenAI({ model: "gpt-4o-mini" }),
  //   allowPartial: false,
  ...
  // console.log(exampleTrimMessages);
...
if (process.argv[1].endsWith("filter-messages-tokens.ts")) {
