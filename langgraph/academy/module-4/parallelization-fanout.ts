...
const node_2 = (state: typeof GraphState.State) => {
  console.log("--- Node 2 ---");
  return {
    graphState: [`I'm node-2!`],
  };
};

// Define `node_2a`, a function that modifies the `graphState`
const node_2a = (state: typeof GraphState.State) => {
  console.log("--- Node 2a ---");
  return {
    graphState: [`I'm node-2a!`],
  };
};
...
const builder = new StateGraph(GraphState)
  ...
  .addNode("node_2a", node_2a) // Add node_2 to the graph
  ...
  .addEdge("node_2", "node_2a")
  .addEdge("node_1", "node_3")
  .addEdge("node_2a", "node_4")
...
if (process.argv[1].endsWith("parallelization-fanout.ts")) {
  main();
}
